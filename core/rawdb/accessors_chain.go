...
func ReadBody(db DatabaseReader, hash common.Hash, number uint64) *types.Body {
	...
	if err := rlp.Decode(bytes.NewReader(data), body); err != nil {
		log.Fatal("Invalid block body RLP", "hash", hash, "err", err)
		return nil
	}
	return body
}
...
func ReadTd(db DatabaseReader, hash common.Hash, number uint64) *big.Int {
	...
	if err := rlp.Decode(bytes.NewReader(data), td); err != nil {
		log.Fatal("Invalid block total difficulty RLP", "hash", hash, "err", err)
		return nil
	}
	return td
}
...
func WriteTd(db DatabaseWriter, hash common.Hash, number uint64, td *big.Int) {
	...
	if err := db.Put(headerTDKey(number, hash), data); err != nil {
		log.Fatal("Failed to store block total difficulty", "err", err)
	}
}
...
func ReadBlock(db DatabaseReader, hash common.Hash, number uint64) *types.Block {
	...
	return types.NewBlockWithHeader(header).WithBody(body.Transactions, body.Votes)
}
